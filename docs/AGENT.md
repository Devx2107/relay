...
The deterministic parser in `lib/agent/intents.ts` recognizes triage requests containing inbox/email/unread or calendar/event language and scheduling requests containing schedule, common scheduling typos, booking, availability, or time-finding language. Help/capability questions and incomplete scheduling requests are answered or clarified locally without provider calls. It extracts an optional bounded triage limit, deduplicated attendee email addresses, and bounded meeting details. Availability checks use only the connected user's primary calendar; attendee addresses are retained for invitations. Unsupported or mixed triage-and-scheduling requests return an `invalid_request` error and never produce an executable action.

SCH-001’s deterministic parser extracts bounded, canonical, deduplicated attendee email addresses and marks missing or display-name attendees as unresolved rather than guessing identities. It also carries bounded location, agenda, and recurrence details into the scheduling proposal. Unsupported, malformed, excessive, or mixed triage-and-scheduling requests return an `invalid_request` error and never produce an executable action. Scheduling parsing performs no provider calls and owns no writes.
