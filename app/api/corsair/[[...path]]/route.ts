...
function handle(method: "GET" | "POST" | "OPTIONS") {
  return async (request: Request) => {
    if (method !== "OPTIONS") {
      ...
      const isHubRequest = 
        request.headers.has("x-corsair-signature") || 
        request.headers.has("corsair-signature");

      // The Corsair CLI needs to read the schema locally to sync with the Hub.
      const url = new URL(request.url);
      const isSchemaFetch = method === "GET" && (url.pathname === "/api/corsair" || url.pathname === "/api/corsair/" || url.pathname === "/api/corsair/openapi.json");
